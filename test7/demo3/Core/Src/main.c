...
void Seg_process(){
	uint16_t voltage = (uint16_t)(v * 100 + 0.5);
	// 分离各位数字
  seg_buf[1] = Seg_Table[voltage / 100];         // 百位(0-3)
  seg_buf[2] = Seg_Table[(voltage / 10) % 10];   // 十位
  seg_buf[3] = Seg_Table[voltage % 10];          // 个位
    
    // 添加小数点(第1位后)
  seg_buf[1] |= 0x80;  // 添加小数点标志(假设您的数码管驱动中最高位控制小数点)
}
void Led_process(){
	if(led1_on == 1){
		led_delay_1000ms++ ;
